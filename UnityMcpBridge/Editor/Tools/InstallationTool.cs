...
namespace UnityMcpBridge.Editor.Tools
{
    public class InstallationTool : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Reset Installation Status", GUILayout.Height(25)))
            {
                if (EditorUtility.DisplayDialog("Confirm Reset", 
                    "This will reset the installation status. Use this only for debugging purposes.", 
                    "Reset", "Cancel"))
                {
                    InstallationManager.ResetInstallationStatus();
                }
            }
