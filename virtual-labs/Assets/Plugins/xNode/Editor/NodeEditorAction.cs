...
namespace XNodeEditor {
    public partial class NodeEditorWindow {
            ...
            switch (e.type) {
                ...
                case EventType.MouseDown:
                    ...
                    if (e.button == 0) {
                        ...
                        else if (IsHoveringNode && (IsHoveringTitle(hoveredNode) || IsHoveringBody(hoveredNode))) {
                ...
                case EventType.MouseUp:
                    if (e.button == 0) {
                        ...
                        if (!isPanning) {
                            ...
                            else if (!IsHoveringNode || hoveredNode is NodeGroup)
                            {
                                autoConnectOutput = null;
                                GenericMenu menu = new GenericMenu();
                ...
                case EventType.ExecuteCommand:
                    ...
                    {
                        if (e.type == EventType.ExecuteCommand) DisplaySelectedNodes();
                        e.Use();
                    }
        ...
        public void Home(XNode.Node node)
        {
            List<XNode.Node> nodes = new List<XNode.Node>();
            nodes.Add(node);
            Vector2 minPos = nodes.Select(x => x.position).Aggregate((x, y) => new Vector2(Mathf.Min(x.x, y.x), Mathf.Min(x.y, y.y)));
            Vector2 maxPos = nodes.Select(x => x.position + (nodeSizes.ContainsKey(x) ? nodeSizes[x] : Vector2.zero)).Aggregate((x, y) => new Vector2(Mathf.Max(x.x, y.x), Mathf.Max(x.y, y.y)));
            panOffset = -(minPos + (maxPos - minPos) / 2f);
        ...
        public void DisplaySelectedNodes()
        {
            XNode.Node[] selectedNodes = Selection.objects.Select(x => x as XNode.Node).Where(x => x != null && x.graph == graph).ToArray();
            if (selectedNodes == null || selectedNodes.Length == 0) return;
        ...
        bool IsHoveringBody(XNode.Node node)
        {
            Vector2 mousePos = Event.current.mousePosition;
            //Get node position
            Vector2 nodePos = GridToWindowPosition(node.position);
            ...
            if (node is NodeGroup)
                ...
                height = node.size.y / zoom;

            Rect windowRect = new Rect(nodePos, new Vector2(width / zoom, height));
            return windowRect.Contains(mousePos);
        }
