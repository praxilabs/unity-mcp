...
namespace XNodeEditor {
    ...
    public static class NodeEditorReflection {
        ...
        [NonSerialized] private static Dictionary<Type, int> nodeOriginalWidth;
        ...
        /// <summary> Set custom node widths defined </summary>
        public static void TrySetAttributeWidth(this Type nodeType, int width)
        {
            if (nodeWidth == null)
            {
                CacheAttributes<int, XNode.Node.NodeWidthAttribute>(ref nodeWidth, x => x.width);
            ...
            nodeWidth[nodeType] = width;
        }

        public static bool TryGetAttributeWidthOriginal(this Type nodeType, out int width)
        {
            if (nodeOriginalWidth == null)
            {
                CacheAttributes<int, XNode.Node.NodeWidthOriginalAttribute>(ref nodeOriginalWidth, x => x.originalWidth);
            }

            return nodeOriginalWidth.TryGetValue(nodeType, out width);
        }

        public static void TrySetAttributeWidthOriginal(this Type nodeType, int width)
        {
            if (nodeOriginalWidth == null)
            {
                CacheAttributes<int, XNode.Node.NodeWidthOriginalAttribute>(ref nodeOriginalWidth, x => x.originalWidth);
            ...
            nodeOriginalWidth[nodeType] = width;
        }
