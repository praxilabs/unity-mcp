...
namespace XNode {
    ...
    public class NodePort {
        ...
        public void VerifyConnections() {
            ...
            ConnectionComparer cc = new ConnectionComparer();
            List<PortConnection> pConnections = new List<PortConnection>();
            for (int i = 0; i < connections.Count; i++)
            {
                if (!pConnections.Contains(connections[i], cc))
                {
                    pConnections.Add(connections[i]);
                }
            }
            connections.Clear();
        ...
        public class ConnectionComparer : IEqualityComparer<PortConnection>
        {
            public bool Equals(PortConnection x, PortConnection y)
            {
                return x.fieldName == y.fieldName
                    && x.Port.node.GetHashCode() == y.Port.node.GetHashCode()
                    && x.Port.fieldName == y.Port.fieldName;
            }

            public int GetHashCode(PortConnection obj)
            {
                return obj.GetHashCode() + obj.Port.node.GetHashCode();
            }
        }
    }
}
